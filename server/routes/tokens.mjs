...
router.post("/pmData", async (req, res) => {
  ...
  try {
    const response = await axios.post(
      "https://www.zohoapis.com/crm/v6/coql",
      {
        select_query: `select id, Created_Time, Est_Move_Date,  Sold_Date, First_Name, Last_Name, Lead_Status, Provider, Internet_Sold, T_V_Sold, Phone_Sold, Move_Ref_Sold, Home_Monitoring, Utilities_set_up, Change_of_Address, New_State, New_City, Call_DispositionX, Electric_Acct, Gas_Acct from Leads where Lead_Source = '${LEAD_Source1}' order by Created_Time desc limit 2000`,
      },
      {
        headers: {
          Authorization: `Zoho-oauthtoken ${access_token}`,
